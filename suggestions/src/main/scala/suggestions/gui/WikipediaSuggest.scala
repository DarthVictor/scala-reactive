...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val suggestions: Observable[Try[List[String]]] = searchTerms.concatRecovered(term => Observable.from(wikipediaSuggestion(term)))
    ...
    // TO IMPLEMENT
    val selections: Observable[String] = button.clicks.filter(_ => suggestionList.selection.items.size > 0)
                                                .map(_ => Observable.from(suggestionList.selection.items)).concat
